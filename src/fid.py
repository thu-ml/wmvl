...
def get_activations_lenet(images, sess, batch_size, verbose):
    assert images.min() < -0.3 and images.max() > 0.3 and images.max() < 0.6
    assert len(images.shape) == 4 and images.shape[-1] == 1
    code_sym = sess.graph.get_tensor_by_name('Relu_5:0')
    inp_ph = sess.graph.get_tensor_by_name('Placeholder_2:0')
    codes = np.zeros((images.shape[0], int(code_sym.shape[-1])))

    from lid.fid_lenet import TqdmUpTo

    n = images.shape[0]
    with TqdmUpTo(unit='img', unit_scale=True, total=n, file=sys.stderr) as t:
        for start in range(0, n, batch_size):
            t.update_to(start)
            end = min(start + batch_size, n)
            inp = images[start:end]
            ...
            out = sess.run(code_sym, {inp_ph: inp})
            codes[start:end] = out[w]
        t.update_to(n)
    return codes
...
def calculate_activation_statistics(images, sess, batch_size=50, verbose=False, model='inception'):
    ...
    if model == 'inception':
        ...
        act = get_activations_lenet(images, sess, 64, verbose)
