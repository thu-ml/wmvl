...
q:
z ~ N(0, 1) # assumed by ais.AIS
x|z ~ N(2z+3, qxz_var)
"""

x_ph = tf.placeholder(tf.float32, [None, 1], name='x')
model = ais.AIS(x_ph, lambda x, z: kde_logpdf(x, generator(z), qxz_var),
                {'input_dim': 1, 'output_dim': 1, 'batch_size': batch_size},
                num_samples, method=method)

xx = np.reshape(x, [batch_size, 1])

schedule = ais.get_schedule(1500, rad=4)
...
plt.plot(x, p1, label='px')
plt.plot(x, np.exp(p2[0]), label='qx_ais')
plt.plot(x, norm.pdf(x, loc=3, scale=(qxz_var+4)**0.5), label='qx_true')
plt.legend()
plt.savefig('/home/ziyu/test-{}.png'.format(method))
